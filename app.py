...
def parse_profile(cv_text):
    ...
    prompt=f'''Extract a truthful candidate profile from the CV below. Never invent information. Required: first_name, last_name, email, phone, and at least one work_experience item. If any required field is missing or unclear, include its exact field name in missing_required. Set profile_ready=true only when all required fields are present. Return JSON exactly like this: {json.dumps(shape)}\n\nCV:\n{cv_text}'''
...
def match_job(profile,job):
    return ai_json('You are a conservative recruitment matching engine. Never invent candidate qualifications.',f'''Score how suitable this job is for the candidate from 0 to 100. Use only candidate facts. Candidate: {json.dumps(profile)} Job: {json.dumps(job)} Return JSON: {{"score":0,"reason":"one short truthful reason"}}''')

def application_draft(profile,job):
    return ai_json('You write concise professional job applications using only supported facts.',f'''Create a short, truthful application email for this opportunity. Candidate: {json.dumps(profile)} Job: {json.dumps(job)} Do not invent qualifications, dates, employers or achievements. Return JSON: {{"subject":"","body":""}}''')
...
def chat():
    try:
        ...
        answer=ai_json('''You are JobHunter AI, a friendly professional career assistant on a job-search website. Answer clearly and briefly. Explain how JobHunter AI works: users upload a CV, AI checks their profile, searches relevant public job sources, scores matches, and prepares tailored applications. Do not promise guaranteed employment, interviews, or actions the system cannot actually perform. Never claim a job application was submitted unless the website explicitly confirms it. Give practical career advice. If asked for a feature the MVP does not have, say so honestly.''',f'''Website context: JobHunter AI is designed to make job seeking less stressful and more hands-off. Candidate profile if available: {json.dumps(profile)} User question: {message} Return JSON: {{"answer":"your answer"}}''')
