...
def chat():
 try:
  ...
  answer=ai_json('''You are JobHunter AI, a friendly professional career assistant. Answer clearly and briefly. Explain that users upload a CV, AI checks their profile, searches relevant public job sources, scores matches, and prepares tailored applications. Never promise employment or claim an application was submitted unless confirmed by the site. Give useful career advice and be honest about features.''',f'''Website: JobHunter AI makes job seeking less stressful. Candidate profile if available: {json.dumps(profile)} User question: {message} Return JSON: {{"answer":"your answer"}}''')
